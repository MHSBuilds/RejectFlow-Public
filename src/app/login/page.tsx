...
export default function LoginPage() {
  ...
  useEffect(() => {
    // Reset loading state when component mounts (handles back button)
    setLoading(false);
    setError('');
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isSignUp) {
        // For sign up, we need to create the user in Supabase Auth first
        // Then NextAuth will handle the session
        ...
        const userExists = checkUserData.exists === true;
        ...
        if (result?.error) {
          if (result.error === 'CredentialsSignin') {
            if (userExists) {
              ...
              setError('No account found with this email. Create an account or continue with Google.');
            }
          } else {
            setError(result.error);
          }
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 via-white to-purple-50 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8">
                 ...
                 <div className="bg-gradient-to-r from-blue-50 to-purple-50 rounded-xl p-4 mb-6 border border-blue-100">
                   <div className="text-center">
                     <h3 className="text-lg font-semibold text-gray-900 mb-2">
                       Transform rejections into valuable feedback candidates deserve
                     </h3>
        ...
        <p className="text-center text-xs text-gray-500">
          By continuing, you agree to RejectFlow's{' '}
          <a 
